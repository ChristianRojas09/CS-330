...
namespace
{
	const char* const WINDOW_TITLE = "Christian Rojas Final Project"; // Macro for window title
	...
	// Textures
	GLuint tableTextureId, boxTextureId, orangeTextureId, bottleTextureId;
...
int main(int argc, char* argv[])
{
	...
	// Load textures
	const char* tableTexFilename = "woodgrain.jpg";
	const char* boxTexFilename = "box.jpg";
	const char* orangeTexFilename = "orange.jpg";
	const char* bottleTexFilename = "bottle.jpg";
	...
	if (!UCreateTexture(boxTexFilename, boxTextureId))
	{
		cout << "Failed to load texture " << boxTexFilename << endl;
		return EXIT_FAILURE;
	}

	if (!UCreateTexture(orangeTexFilename, orangeTextureId))
	{
		cout << "Failed to load texture " << orangeTexFilename << endl;
		return EXIT_FAILURE;
	}
	...
	UDestroyTexture(orangeTextureId);
...
#pragma region box Binding / Generation

	float boxScale = 0.5f;
	glm::mat4 scale = glm::scale(glm::vec3(boxScale, boxScale, boxScale));
...
#pragma region Bottle Binding / Generation
	...
	modelLoc = glGetUniformLocation(gProgramId, "model");
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	static_meshes_3D::Cylinder C1(1, 20, 40, true, true, true);
	C1.render();

#pragma endregion

#pragma region Orange Binding / Generation
	Sphere Orange(0.1f, 32, 32);
	glActiveTexture(GL_TEXTURE0);
	...
	model = glm::scale(model, glm::vec3(1.2f, 5.03f, 2.2f));
...
void UCreateMesh(GLMesh& mesh)
{
	...
	mesh.nVertices[0] = sizeof(boxVerts) / (sizeof(boxVerts[0]) * (floatsPerVertex + floatsPerNormal + floatsPerUV));
